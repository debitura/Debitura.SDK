...
class DebituraWebExternalApiContractsV1ChatsSendChatRequest(BaseModel):
    """
    Request to send a chat message on a case.
    """ # noqa: E501
    message: Annotated[str, Field(min_length=0, strict=True, max_length=10000)] = Field(description="The chat message content to send.")
    user_id: Optional[StrictStr] = Field(default=None, description="ID of the team member sending this message. Must be a valid user ID from GET /users belonging to your team. Deprecated: use UserEmail instead. At least one of UserId or UserEmail must be provided.", alias="userId")
    user_email: Optional[StrictStr] = Field(default=None, description="Email address of the team member sending this message. Must correspond to an active member of your team (same account as the API key). Preferred over UserId. At least one of UserEmail or UserId must be provided.", alias="userEmail")
    __properties: ClassVar[List[str]] = ["message", "userId", "userEmail"]
