...
class DebituraWebExternalApiContractsV1LeadsLeadListDto(BaseModel):
    """
    V1 Lead list response for external customer APIs
    """ # noqa: E501
    quote_requests: Optional[List[DebituraWebExternalApiContractsV1LeadsLeadDto]] = Field(alias="quoteRequests")
    __properties: ClassVar[List[str]] = ["quoteRequests"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in quote_requests (list)
        _items = []
        if self.quote_requests:
            for _item in self.quote_requests:
                if _item:
                    _items.append(_item.to_dict())
            _dict['quoteRequests'] = _items
        # set to None if quote_requests (nullable) is None
        # and model_fields_set contains the field
        if self.quote_requests is None and "quote_requests" in self.model_fields_set:
            _dict['quoteRequests'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "quoteRequests": [DebituraWebExternalApiContractsV1LeadsLeadDto.from_dict(_item) for _item in obj["quoteRequests"]] if obj.get("quoteRequests") is not None else None
        })
        return _obj
