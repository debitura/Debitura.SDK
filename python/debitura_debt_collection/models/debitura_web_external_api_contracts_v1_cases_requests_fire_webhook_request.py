...
class DebituraWebExternalApiContractsV1CasesRequestsFireWebhookRequest(BaseModel):
    """
    Request body for POST /test/webhooks/fire.
    """ # noqa: E501
    event: Optional[StrictStr] = Field(description="The webhook event type to fire (e.g. \"case.updated\", \"payment.created\"). Must be a valid event type for the calling API — refer to each API's documentation for the supported list (Collection Partner and Customer API accept different sets).")
    case_id: StrictStr = Field(description="The ID of the test case to use as the event source.", alias="caseId")
    subscription_id: Optional[StrictStr] = Field(default=None, description="Optional — the specific subscription ID to fire to. If omitted, fires to all active test subscriptions for the calling API subscriber that are registered for the given event type.", alias="subscriptionId")
    __properties: ClassVar[List[str]] = ["event", "caseId", "subscriptionId"]
