# coding: utf-8

"""
    Debitura Customer API

    Welcome to the Debitura Customer API, your gateway to seamlessly integrating your debt collection workflow with our platform. Debitura connects customers with trusted debt collection firms specializing in pre-legal debt recovery. With this API, you can easily upload cases, track their status, and manage your debt collection process efficiently. For any support or inquiries, please reach out to us at contact@debitura.com. To setup an API key, log into your Debitura account and go here: https://app.debitura.com/CreditorApiKey  📖 Full documentation, guides, and integration walkthroughs: https://docs.debitura.com/clients  NOTE: if you're representing a platform and want to provide this feature for your customers, you need a different API. Please contact us.

    The version of the OpenAPI document: v1
    Contact: contact@debitura.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from debitura_debt_collection.models.debitura_web_external_api_contracts_v1_tasks_task_dto import DebituraWebExternalApiContractsV1TasksTaskDto

class TestDebituraWebExternalApiContractsV1TasksTaskDto(unittest.TestCase):
    """DebituraWebExternalApiContractsV1TasksTaskDto unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> DebituraWebExternalApiContractsV1TasksTaskDto:
        """Test DebituraWebExternalApiContractsV1TasksTaskDto
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `DebituraWebExternalApiContractsV1TasksTaskDto`
        """
        model = DebituraWebExternalApiContractsV1TasksTaskDto()
        if include_optional:
            return DebituraWebExternalApiContractsV1TasksTaskDto(
                id = '',
                type = '',
                type_label = '',
                title = '',
                description = '',
                status = '',
                case_id = '',
                case_reference = '',
                lead_id = '',
                deadline = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                date_created = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                date_solved = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                solution_url = '',
                action = debitura_debt_collection.models.debitura/web/external_api/contracts/v1/tasks/task_action_dto.Debitura.Web.ExternalApi.Contracts.V1.Tasks.TaskActionDto(
                    method = '', 
                    path = '', )
            )
        else:
            return DebituraWebExternalApiContractsV1TasksTaskDto(
        )
        """

    def testDebituraWebExternalApiContractsV1TasksTaskDto(self):
        """Test DebituraWebExternalApiContractsV1TasksTaskDto"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
